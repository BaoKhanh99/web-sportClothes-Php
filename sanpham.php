...
$output = '';
$query = "SELECT * FROM sanpham WHERE id_sp = '$idsp' ";
$sql = mysqli_query($con,$query);
$num = mysqli_num_rows($sql);
while ($row = mysqli_fetch_assoc($sql)) {
	$output .= '
	<div class="col-3 mt-4" >
	<img src="img/'.$row["danhmuc"].'/'.$row["hinhanh"].'" alt="" width="70%" class="text-center ml-5">
	</div>
	<div class="col-8 mt-4">
	<h3>'.$row["ten_sp"].'</h3>
	<h5>'.$row["gia"].'đ</h5>
	'; 
	$_SESSION['tensp'] = $row["ten_sp"];
	$_SESSION['hinhanh'] = $row["hinhanh"];
	$_SESSION['danhmuc'] = $row["danhmuc"];
	...
	<div class="container-fluid" >
		...
		<div class="row">
			...
			<form action="" id="form_cart">
				<div class="soluong mt-4">
					Số Lượng <input type="text" id="soluong" name ="soluong" size="5">
				</div>
				<div class="btns mt-4">
					<button class="btn btn-primary btn-lg mr-2" id="add_cart">Thêm vào giỏ hàng</button>
				</div>
			</form>
			<div id="#div1"></div>
		</div>
