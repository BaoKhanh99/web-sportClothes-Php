...
{
	$search = addslashes($_GET['input_search']);
	if (empty($search)) {
		$error = "Yêu cầu nhập dữ liệu vào ô trống";
	} 
	else
	{
		for ($i=0; $i <2 ; $i++) { 
			$output .='<div class="row mt-4">';
			$query = "SELECT * FROM sanpham WHERE ten_sp LIKE '%$search%' UNION SELECT * FROM sanpham LIMIT $l,$limit  ";
	...
	<div class="container-fluid" >
		...
		<div class="row">
			...
			<div class="col-8 mt-4">
				...
				<div class="product mt5 " id="ops"></div>
				</div>
		</div>	
	</div>
